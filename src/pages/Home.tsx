  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-12 sm:px-10">
      <header className="mb-12">
        ...
        <h1 className="max-w-3xl text-4xl font-extrabold leading-tight text-white sm:text-5xl">{COURSE.title}</h1>
        <p className="mt-4 max-w-2xl text-lg leading-relaxed text-white/55">{COURSE.subtitle}</p>

        <dl className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {STATS.map((s) => (
            <div key={s.label} className="card px-4 py-3.5">
              <dd className="text-2xl font-bold text-white">{s.value}</dd>
              <dt className="mt-0.5 text-xs text-white/40">{s.label}</dt>
            </div>
          ))}
        </dl>
      </header>
      ...
      <section>
        <h2 className="mb-5 text-lg font-bold text-white/80">Lộ trình 8 buổi</h2>
        <div className="grid gap-3.5 sm:grid-cols-2">
          {BUOI_LIST.map((b) => (
            <Link
              key={b.id}
              to={`/buoi/${b.slug}`}
              className="card group flex flex-col p-5 transition hover:border-brand-400/40 hover:bg-white/[0.07]"
            >
              <div className="mb-3 flex items-center gap-2.5">
                ...
                <span className="text-[11px] uppercase tracking-wider text-white/30">{b.duration}</span>
              </div>

              <h3 className="text-[15px] font-bold leading-snug text-white transition group-hover:text-brand-300">{b.title}</h3>
              <p className="mt-1.5 flex-1 text-[13px] leading-relaxed text-white/45">{b.subtitle}</p>

              <div className="mt-4 flex flex-wrap gap-1.5">
                {b.keywords.slice(0, 4).map((k) => (
                  <span key={k} className="rounded-md bg-white/6 px-2 py-0.5 font-mono text-[10.5px] text-white/45">
                    {k}
                  </span>
                ))}
              </div>
      ...
      <section className="mt-12">
        <h2 className="mb-4 text-lg font-bold text-white/80">Cách dùng trang này</h2>
        <ul className="card space-y-2.5 p-5 text-[14px] leading-relaxed text-white/60">
