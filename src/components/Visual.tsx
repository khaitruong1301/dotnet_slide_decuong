...
const NODE_STYLE: Record<FlowStep['kind'], string> = {
  ...
  note: 'bg-white/4 border-dashed border-white/25 text-white/60 rounded-lg',
}
...
function Arrow({ h = 30 }: { h?: number }) {
  return (
    <div className="flex flex-col items-center shrink-0" style={{ height: h }} aria-hidden>
      ...
      <div className="border-x-4 border-x-transparent border-t-[6px] border-t-white/35" />
    </div>
  )
}
                ...
                <div className="flex items-start gap-4">
                  {step.branches.map((b, bi) => (
                    <div key={bi} className="flex-1 flex flex-col items-center">
                      <span className="-mt-8 mb-1 rounded-full border border-white/15 bg-ink-900 px-2.5 py-0.5 text-[0.66em] font-semibold uppercase tracking-wider text-white/60">
                        {b.label}
                      </span>
                      <Arrow h={18} />
...
function Caption({ text }: { text?: string }) {
  if (!text) return null
  return <p className="mt-4 text-center text-[0.8em] text-white/45">{text}</p>
}
...
function Panel({ title, tone, items }: { title: string; tone: string; items: string[] }) {
  return (
    <div className={`flex-1 rounded-xl border p-4 ${tone}`}>
      ...
      <ul className="space-y-1.5 text-[0.86em] leading-snug text-white/80">
        {items.map((t, i) => (
...
function Chevron() {
  return (
    <div className="hidden shrink-0 self-center text-white/30 sm:block" aria-hidden>
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {v.items.map((it, i) => (
              <div key={i} className="text-center">
                ...
                <div className="min-w-[7rem] rounded-lg border border-white/15 bg-white/5 px-4 py-3 font-mono text-[0.95em] text-white">
                  {it.value}
                </div>
                {it.note && <div className="mt-1.5 text-[0.7em] text-white/45">{it.note}</div>}
              </div>
            ))}
          </div>
          <Caption text={v.caption} />
                  ...
                  <div key={i} className="text-center">
                    <div
                      className={`flex h-14 min-w-[4.2rem] items-center justify-center border px-3 font-mono text-[0.92em] ${
                        on ? 'border-brand-400 bg-brand-500/25 text-white' : 'border-white/15 bg-white/5 text-white/85'
                      } ${i === 0 ? 'rounded-l-lg' : ''} ${i === v.items.length - 1 ? 'rounded-r-lg' : 'border-r-0'}`}
                    >
                      {it}
                    </div>
                    <div className="mt-1.5 font-mono text-[0.68em] text-white/40">[{i}]</div>
                  </div>
                )
              })}
            </div>
          ...
          <div className="mx-auto max-w-2xl space-y-2">
            {v.pairs.map((p, i) => (
              <div key={i} className="flex items-center gap-3">
                ...
                <svg width="30" height="14" viewBox="0 0 30 14" className="shrink-0 text-white/35" fill="none" stroke="currentColor" strokeWidth="1.6" aria-hidden>
                  <path d="M1 7h24M20 2l6 5-6 5" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
                <div className="flex-1 rounded-lg border border-white/15 bg-white/5 px-3 py-2 font-mono text-[0.85em] text-white/85">{p.value}</div>
              </div>
            ))}
          </div>
          <Caption text={v.caption} />
              ...
              {[
                ...
                <div key={i} className="flex items-center gap-2">
                  <div className={`rounded-lg border px-3.5 py-2.5 text-center ${b.c}`}>
                    ...
                    <div className="mt-1 font-mono text-white/90">{b.v}</div>
                  </div>
                  {i < arr.length - 1 && <span className="text-white/30">→</span>}
                </div>
              ))}
            </div>
            <div className="pointer-events-none absolute inset-x-8 bottom-0 h-8 rounded-b-xl border-x border-b border-dashed border-white/25" aria-hidden />
            <div className="absolute inset-x-0 bottom-0 translate-y-1/2 text-center">
              <span className="rounded-full border border-white/15 bg-ink-900 px-3 py-0.5 text-[0.7em] text-white/55">
        ...
        <div className="w-full">
          <div className="flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            <div className="space-y-1.5">
              ...
              <div className="text-center text-[0.68em] uppercase tracking-widest text-white/40">tham số</div>
            </div>
            <Chevron />
            <div className="rounded-xl border border-brand-400/50 bg-brand-500/12 px-5 py-4 text-center">
              ...
              <div className="mt-1.5 max-w-[16rem] text-[0.8em] leading-snug text-white/65">{v.body}</div>
            </div>
            <Chevron />
            <div>
              ...
              <div className="mt-1 text-center text-[0.68em] uppercase tracking-widest text-white/40">giá trị trả về</div>
            </div>
          </div>
          <Caption text={v.caption} />
        </div>
          ...
          <ol className="mx-auto max-w-3xl space-y-3">
            {v.items.map((it, i) => (
              <li key={i} className="flex gap-3.5">
                ...
                <div className="flex-1 rounded-lg border border-white/10 bg-white/4 px-3.5 py-2.5">
                  <div className="text-[0.88em] font-semibold text-white/90">{it.label}</div>
                  <div className="mt-0.5 text-[0.82em] leading-snug text-white/60">{it.text}</div>
                </div>
              </li>
            ))}
          </ol>
